...
async function async_test(a, b, res) {
  return new Promise((resolve) => {
    if ((a + b) === res) {
      resolve(`Correct: ${a} + ${b} = ${res}`);
    } else {
      resolve(`Incorrect: ${a} + ${b} != ${res}`);
    }
  });
}
...
async function checkResult(e: WebUI.Event) {
  ...
  const async_msg = await async_test(a, b, res); // async test

  await Deno.writeTextFile("msg.txt",async_msg);
  console.log("msg.txt written on disk")
  const msg_from_file = await Deno.readTextFile("msg.txt");
  console.log("read result: " + msg_from_file)
